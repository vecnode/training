...
class CheckpointTrackerCallback(TrainerCallback):
    ...
    def _write_tracking_files(self, state: TrainerState) -> None:
        ...
        if latest_ckpt:
            resume_cmd = (
                "../.venv/Scripts/python.exe train_qwen3b_lora.py "
...
# Matches build_qwen3b_dataset.py's CNN_DAILYMAIL_PROMPT_INSTRUCTION, so it
...
# Qwen2.5-Instruct's chat format (ChatML), not Vicuna's "USER:/ASSISTANT:".
# Verified against Qwen/Qwen2.5-3B-Instruct's tokenizer_config.json: eos_token
# is "<|im_end|>" (used to close the assistant turn below), chat_template uses
# <|im_start|>{role}\n...<|im_end|> blocks.
...
class SummaryCollator:
    """Source-text -> summary collator for a plain causal LM (Qwen2.5-3B).

    No image concepts apply here - this loads only Qwen2.5-3B-Instruct's own
    weights, no vision components anywhere. Uses Qwen's ChatML wrapper
    (<|im_start|>role\\n...<|im_end|>) instead of Vicuna's "USER:/ASSISTANT:".
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Qwen2.5-3B LoRA training (text summarization)")
    parser.add_argument("--dataset-jsonl", type=Path, default=_TRAINING_DIR / "data" / "qwen3b_train.jsonl", help="Path to JSONL created by build_qwen3b_dataset.py")
    parser.add_argument("--output-dir", type=Path, default=_TRAINING_DIR / "runs" / "qwen3b_lora", help="Where checkpoints and final adapter are saved")
    parser.add_argument("--model-id", default="Qwen/Qwen2.5-3B-Instruct", help="Hugging Face model id")
    ...
    parser.add_argument("--max-length", type=int, default=2048, help="Max tokenized sequence length")
    ...
    parser.add_argument("--instruction", default=DEFAULT_INSTRUCTION, help="Instruction prefix prepended to every example (must match generation, see generate_qwen3b_lora.py --instruction)")
