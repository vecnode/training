...
# (train_qwen3b_lora.py's DEFAULT_INSTRUCTION). Override with --instruction
...
# Qwen2.5-Instruct's chat format (ChatML) - see train_qwen3b_lora.py for the
# verification notes (tokenizer_config.json eos_token/chat_template).
CHATML_SYSTEM = "You are a helpful assistant."
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate summaries from source text with a trained Qwen2.5-3B LoRA adapter")
    parser.add_argument("--adapter-dir", type=Path, default=_TRAINING_DIR / "runs" / "qwen3b_lora" / "final_adapter", help="Path to trained LoRA adapter directory")
    ...
    parser.add_argument("--reference-csv", type=Path, default=None, help="Optional reference summaries CSV for token-F1")
    parser.add_argument("--out-csv", type=Path, default=_TRAINING_DIR / "runs" / "qwen3b_lora" / "generated.csv", help="Output CSV with generated predictions (CSV mode)")
    ...
    parser.add_argument("--eval-seed", type=int, default=42, help="Must match training's --seed so the held-out split lines up (train_qwen3b_lora.py default: 42)")
    parser.add_argument("--eval-val-ratio", type=float, default=0.1, help="Must match training's --val-ratio so the held-out split lines up (train_qwen3b_lora.py default: 0.1)")
...
class Summarizer:
    ...
    def __init__(self, adapter_dir: Path, base_model_id: str, max_length: int, max_new_tokens: int, instruction: str = DEFAULT_INSTRUCTION):
        ...
        # Precompute the fixed ChatML wrapper so only the source body is
        # re-tokenized per call.
        head = f"<|im_start|>system\n{CHATML_SYSTEM}<|im_end|>\n<|im_start|>user\n{instruction}"
        suffix = "<|im_end|>\n<|im_start|>assistant\n"
        self.head_ids = self.tokenizer(head, add_special_tokens=True).input_ids
        self.suffix_ids = self.tokenizer(suffix, add_special_tokens=False).input_ids
...
def run_jsonl_eval(summarizer: Summarizer, args: argparse.Namespace) -> int:
    ...
    Replicates train_qwen3b_lora.py's split (same seed + val_ratio applied to
