...
for VAE training: a (n, 3, 32, 32) [0,1]-scaled float32 tensor X and an
int64 label vector y per split, plus the 10 class names. Labels are saved
only so evaluate_vae.py can pick one example per class for the
reconstruction grid - the VAE itself never sees them.
