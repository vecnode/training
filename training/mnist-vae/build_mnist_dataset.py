"""
Turn the raw MNIST IDX ubyte files into numpy arrays ready for VAE
training: a flattened-then-reshaped [0,1]-scaled pixel tensor X and a label
vector y (labels are saved only so evaluate_vae.py can pick one example per
digit for the reconstruction grid - the VAE itself never sees them).
...
in training/mnist-kmeans - each uv project here is independent and doesn't
import another pipeline folder's code.
