"""
Turn the raw CIFAR-100 python-format pickles into numpy arrays ready for MAE
pretraining: a (n, 3, 32, 32) [0,1]-scaled float32 tensor X and, per split,
the fine (100-class) and coarse (20-superclass) label vectors, plus both
name lists. Same data contract as training/vit-cifar10 and
training/cifar10-vqvae (which carry CIFAR-10's twin format): the trainer
keeps raw [0,1] pixels and normalizes per-channel itself (hardcoded
train-set statistics), so the .npz stores raw pixels like the siblings.
...
Usage:
    uv run --directory training/mae-cifar100 python build_cifar100_dataset.py \
