"""
Turn the raw MNIST IDX ubyte files into numpy arrays ready for
flow-matching training: a [0,1]-scaled pixel tensor X and a label vector y
(labels are saved only so evaluate_flow.py can pick one example per digit
for the round-trip grid - the flow model itself is unconditional and never
sees them).

The output npz is deliberately the *same contract* as training/mnist-vae's
(X_train/y_train/X_test/y_test, float32 in [0,1]), so the two pipelines can
be compared on byte-identical data. Flow matching wants inputs centered
around the N(0,I) noise it transports from, so the [0,1] -> [-1,1] rescale
is done explicitly in train_flow.py rather than being baked in here.
...
the stdlib struct module. This is a standalone copy of the same parser used
in training/mnist-kmeans and training/mnist-vae - each uv project here is
independent and doesn't import another pipeline folder's code.
