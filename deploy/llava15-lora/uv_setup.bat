...
rem it, `uv sync` also prunes torch/torchvision themselves despite the
rem --no-install-package flags, undoing the CUDA wheel install below.
echo.
...
endlocal & set "LLAVA_DEPLOY_PYTHON=%VENV_PY%" & exit /b 0
